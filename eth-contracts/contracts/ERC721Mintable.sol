...
//  4) create 'whenNotPaused' & 'paused' modifier that throws in the appropriate situation
  modifier whenNotPaused(){
    require(_paused == false, 'The contract has been paused');
    _;
  }

  modifier paused(){
    require(_paused == true, 'The contract has unpaused');
    _;
  }
...
//    @dev Approves another address to transfer the given token ID
    function approve(address to, uint256 tokenId) public {
      require(to != address(0), "the target address should not be empty");
        // TODO require the given address to not be the owner of the tokenId
      require(to != ownerOf(tokenId), "the target address should not be the owner of the tokenId");
        // TODO require the msg sender to be the owner of the tokenId or isApprovedForAll() to be true
      require(msg.sender == ownerOf(tokenId) || isApprovedForAll(ownerOf(tokenId), msg.sender), "the sender must be the owner of the tokenId or the approved operator");
    ...
    function transferFrom(address from, address to, uint256 tokenId) public {
      require(_isApprovedOrOwner(msg.sender, tokenId), "only owner can transfer the token");
    ...
    function _transferFrom(address from, address to, uint256 tokenId) internal {
      ...
      require(from == ownerOf(tokenId), "from address must be the owner of the tokenId");
        // TODO: require token is being transfered to valid address
      require(to != address(0), "to address must not be empty");
        // TODO: clear approval
...
contract ERC721Metadata is ERC721Enumerable, usingOraclize {
  ...
  function _getBaseTokenURI() public view returns (string memory) {
...
//      -calls the superclass mint and setTokenURI functions
  function mint(
		address to,
		uint256 tokenId
		// There is no need to pass in tokenURI, since it will be decided based on tokenId and hardcoded baseTokenURI anyway
		/* ,string tokenURI */
